...
public abstract class BaseLauncher
{
    ...
    /// <summary>
    /// Path to MultiMC.exe or PolyMC.exe or PrismLauncher.exe or FjordLauncher.exe
    /// </summary>
    public abstract string ExecutablePath { get; }

    /// <summary>
    /// Path to multimc.cfg or polymc.cfg or prismlauncher.cfg or fjordlauncher.cfg
    /// </summary>
    public abstract string ConfigPath { get; }

    /// <summary>
    /// Name of the running process for MultiMC or PolyMC or PrismLauncher or FjordLauncher
    /// </summary>
...
public sealed class FjordLauncher : BaseLauncher
{
    public override string Name => "FjordLauncher";
    public static string RelativeExecutablePath => "fjordlauncher.exe";
    public override string ExecutablePath => Path.Combine(InstallDirectory, RelativeExecutablePath);
    public override string ConfigPath => Path.Combine(DataDirectory, "fjordlauncher.cfg");
    public override string ProcessName => "FjordLauncher";
    public override string IconName => "icon-fjordlauncher.png";
    public override string DataDirectory { get; }
    public FjordLauncher(string installDirectory) : base(installDirectory)
    {
        var isPortable = File.Exists(Path.Combine(installDirectory, "portable.txt"));
        DataDirectory = isPortable
            ? InstallDirectory
            : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "FjordLauncher"); // TODO test as admin
...
public class MultiMcLibrary : LibraryPlugin
    {
        ...
        public override string Name => "MultiMC/PolyMC/Prism/Fjord";
        ...
        private void DetectLauncher()
        {
            ...
            else if (File.Exists(Path.Combine(rootFolder, FjordLauncher.RelativeExecutablePath)))
            {
                Launcher = new FjordLauncher(rootFolder);
            }
            else
            {
                Launcher = null;
